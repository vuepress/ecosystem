...
export interface ReplacementRule {
   ...
   *
   *   - Strings starting with `^` or ending with `$` are automatically converted to
   *     regular expressions
...
export interface ReplaceAssetsOptions {
  ...
  /**
   * Built-in image matching rules, designed to match and find common image
   * paths starting with `^/images/`
   *
  ...
  image?: Replacement
  /**
   * Built-in media matching rules, designed to match and locate common media
   * paths such as videos and audio that start with `^/medias/`.
   *
  ...
  media?: Replacement
  /**
   * Equivalent to setting both {@link ReplaceAssetsOptions.image image} and
   * {@link ReplaceAssetsOptions.media media} simultaneously.
   *
   * 相当于同时设置 {@link ReplaceAssetsOptions.image image} 和
   * {@link ReplaceAssetsOptions.media media}
   *
   * @example
   *   ;({
