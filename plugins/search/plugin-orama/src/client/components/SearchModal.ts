...
const SearchResult = defineAsyncComponent({
  loader: () =>
    import(/* webpackChunkName: "orama-result" */ './SearchResult.js'),
  loadingComponent: () => {
    const localeConfig = useLocale(locales)

    return h(SearchLoading, {
      class: 'orama-result-wrapper',
...
export default defineComponent({
  ...
  setup() {
    ...
    return (): VNode | null =>
      isActive.value
        ? h('div', { class: 'orama-modal-wrapper' }, [
            h('div', {
              class: 'orama-mask',
              onClick: () => {
                toggleActive(false)
            ...
            h('div', { class: 'orama-modal' }, [
              h('div', { class: 'orama-box' }, [
                h('form', [
                  h(
                    'label',
                    {
