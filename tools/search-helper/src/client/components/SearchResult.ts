...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const locale = useLocale(getSearchClientConfig().locales)
    const {
    ...
    const getDisplay = (matchedItem: MatchedItem): (VNode | string)[] => {
      if (matchedItem.type === 'customField') {
        const formatterConfig =
          getSearchClientConfig().customFieldConfig?.[matchedItem.index]
    ...
    const renderSearchQueryHistory = (): VNode | null =>
      enableQueryHistory
        ? h(
            'div',
            { class: 'vp-search-records' },
            h('div', { class: 'vp-search-record' }, [
              h(
                'div',
                { class: 'vp-search-record-title' },
                locale.value.queryHistory,
              ),
              h(
                'ul',
                { class: 'vp-search-record-contents', role: 'listbox' },
                queryHistories.value.map((item, historyIndex) => {
                  ...
                  return h(
                    'li',
                    {
                      'class': ['vp-search-record-matches', { active }],
                      'role': 'option',
                    ...
                    h('div', [
                      h(HistoryIcon, {
                        class: 'vp-search-record-type',
                      }),
                      h('div', { class: 'vp-search-record-content' }, item),
                      h('button', {
                        'type': 'button',
                        'class': 'vp-search-remove-icon',
                  ...
                  return h(
                    ...
                    h(
                      ...
                      () => [
                        ...
                        h('div', { class: 'vp-search-record-content' }, [
                          item.header
                            ? h(
                                'div',
                                { class: 'vp-search-record-content-header' },
                                item.header,
                              )
                            : null,
                          h(
    ...
    watch(
      ...
      () => {
        document
          .querySelector(
            '.vp-search-record.active .vp-search-record-matches.active',
          )
    ...
    return (): VNode =>
      h(
        'div',
        {
          id: 'vp-search-results',
          class: [
            'vp-search-result-wrapper',
            {
              empty:
            ...
            : hasResults.value
              ? h(
                  'div',
                  {
                    'class': 'vp-search-records',
                    'role': 'listbox',
                    'aria-labeledby': 'vp-search-label',
                  },
                  results.value.map(({ title, contents }, index) => {
                    ...
                    return h(
                      ...
                      [
                        ...
                        h(
                          'ul',
                          { class: 'vp-search-record-contents' },
                            ...
                            return h(
                              ...
                              h(
                                ...
                                () => [
                                  ...
                                  h(
                                    'div',
                                    { class: 'vp-search-record-content' },
                                    [
                                      item.type === 'text' && item.header
                                        ? h(
                                            ...
                                            {
                                              class:
                                                'vp-search-record-content-header',
                                            },
                                            item.header,
                                          )
                                        : null,
